...
# 测试：不需要 GPU 或在线模型；原生视频解码测试需要 vLLM 环境。
/opt/conda/envs/vln/bin/python -m unittest discover -s dataset_create/instruction/tests -v
/opt/conda/envs/vllm/bin/python -m unittest dataset_create.instruction.tests.test_video_transport -v
```
...
Instruction 阶段直接使用 Habitat-Sim，没有运行 Habitat-Lab 的任务 measures，也不重复重建场景的导航区域分区。发现首个失败片段后先修复，再检查后续片段；没有普通运动文本时跳过该项模型调用，入库仍要求全部决策、普通运动和停止检查通过。独立观察会增加首次决策审核的调用，但文字修复复用同一份观察；实际吞吐还取决于模型服务负载。
...
3. **局部语言生成**：Qwen 描述 navigator 应该怎么走。视觉细节服务于识别入口、路径或停止位置；选择依据在作出选择时可用。可参考上一段的称呼，当前路径仍由当前视觉证据决定。输出保留局部文本与 segment 的关系。
4. **轻量整理**：改善衔接、合并重复，不增加路线事实。关键选择和停止 clause 的原文、顺序及来源由程序检查；最终 instruction 不依赖路线标记。
5. **局部验证与修复**：先在不提供 instruction 的情况下观察 clean 决策区域，再分别检查文字与实际路径是否一致、是否足以区分入口。普通运动和停止单独检查。失败定位到相关 segment，优先修改局部语言；作者认为材料或切分不足时返回上游。默认每段最多两次文字修复、一次上游修复，仍无法明确表达或验证的轨迹隔离。
...
视频按位移和累计转角选帧，保留片段边界、决策和真实 STOP。核心视频仅包含当前段。决策审核视频从该决策的接近过程开始，保留到自然片段末尾，避免把实际转向或入口过程截断；它有意展示实际通行结果，用于路线一致性审核。选定帧通过 vLLM OpenCV 解码器全部传入，关闭模型处理器的再次选帧。

### 验证标准

- **决策**：观察任务只接收 clean 决策区域视频、选择前 compass 和相对姿态，不接收 instruction、GT 分支答案或路线标记。它记录实际通行顺序、选择前可用的线索及其他入口。随后文字审核分别判断 `path_matches` 与 `choice_is_clear`，两项均为真、状态通过且置信度足够才入库。含糊描述即使与实际视频相容，也不能直接通过。观察不随局部文案变化，可由请求缓存复用。
- **终点**：使用 clean 接近视频与真实 stop compass，检查文字是否指向同一局部目的地，是否会明显停早或走过头。不要求区分同一目的地区域内的细小站位差别。
- **普通运动**：用 clean 视频及相对朝向、高度检查会误导导航的错误，例如左右写反或上下楼错误。已由入口、停止任务覆盖的 clause 不重复检查；不要求逐帧复述动作。
