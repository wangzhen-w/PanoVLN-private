...
class PanoworldSupervisedDataset(Dataset):
    def __init__(
        ...
        self._fp = None
        self._configure_image_processor(image_processor_cfg or {})
    ...
    def _configure_image_processor(self, image_processor_cfg: Dict[str, Any]) -> None:
        image_processor = getattr(self.processor, "image_processor", None)
        if image_processor is None:
            return
        for key, value in image_processor_cfg.items():
            if key == "size" and isinstance(value, dict):
                current_size = getattr(image_processor, "size", None)
                if isinstance(current_size, dict):
                    current_size.update({k: v for k, v in value.items() if v is not None})
                else:
