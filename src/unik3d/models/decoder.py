...
class RadialModule(nn.Module):
    ...
    def _process_decoder_features(self, features, rays_hr):
        rays_embeddings = self.embed_rays(rays_hr)
        return self.process(features, rays_embeddings)

    def extract_decoder_features(self, features, rays_hr):
        decoder_features, _ = self._process_decoder_features(features, rays_hr)
    ...
    def forward(
        ...
        features, lowres_features = self._process_decoder_features(features, rays_hr)
...
class Decoder(nn.Module):
    ...
    def _prepare_decoder_inputs(self, inputs, *, build_auxiliary_embeddings=True):
        ...
        pos_embed = None
        level_embed = None
        if build_auxiliary_embeddings:
            # Retained for the original depth-forward API even though the radial module
            # currently ignores both values.
        ...
        return features, rays, intrinsics, pos_embed, level_embed

    def extract_decoder_features(self, inputs):
        features, rays, _, _, _ = self._prepare_decoder_inputs(
            inputs,
            build_auxiliary_embeddings=False,
        )
        return self.radial_module.extract_decoder_features(
            features=features,
            rays_hr=rays,
        )

    def forward(self, inputs, image_metas) -> torch.Tensor:
        features, rays, intrinsics, pos_embed, level_embed = self._prepare_decoder_inputs(
            inputs
        )
