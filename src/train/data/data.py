...
DEFAULT_DAP_IMAGE_SIZE = (1024, 512)
...
def preprocess_dap_current_image(image: Image.Image) -> torch.Tensor:
    processed_image = image.convert("RGB").resize(
        DEFAULT_DAP_IMAGE_SIZE,
...
class SupervisedDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        if self.dap_enabled and vision_paths:
            with Image.open(vision_paths[-1]) as image:
                item["dap_pixel_values"] = preprocess_dap_current_image(image).unsqueeze(0)
