...
def add_residual(x, brange, residual, residual_scale_factor, scaling_vector=None):
    if scaling_vector is None:
        ...
        x_plus_residual = scaled_index_add(
            x, brange, residual.to(dtype=x.dtype),
            scaling=scaling_vector, alpha=residual_scale_factor,
        )
...
def get_attn_bias_and_cat(x_list, branges=None):
    """Concatenate sequences and provide block-diagonal mask for xFormers nested path."""
    batch_sizes = (
...
class Block(nn.Module):
    """Standard self-attention + FFN block."""
...
class NestedTensorBlock(Block):
    """xFormers nested-tensor path for Block."""
...
class BlockRope(nn.Module):
    """Self-attention + FFN block with RoPE positional encoding."""
    ...
    def forward(self, x: Tensor, xpos=None) -> Tensor:
        ...
        Args:
            ...
            xpos: RoPE 2D position indices, shape (B, N, 2).
        """
        def attn_residual_func(y: Tensor) -> Tensor:
            return self.ls1(self.attn(self.norm1(y), xpos=xpos))

        def ffn_residual_func(y: Tensor) -> Tensor:
            return self.ls2(self.mlp(self.norm2(y)))
...
class CrossBlockRope(nn.Module):
    """Self-attention + cross-attention + FFN block with RoPE positional encoding."""
    ...
    def __init__(
        ...
        self.attn = attn_class(
            dim, num_heads=num_heads, qkv_bias=qkv_bias,
            proj_bias=proj_bias, rope=rope, qk_norm=qk_norm,
        )
        # Cross-attention
    ...
    def forward(
        ...
        """
        Args:
            x: (B, Nx, C) query sequence (also self-attention input).
            y: (B, Ny, C) context sequence (cross-attention key/value).
            xpos: RoPE position indices for x.
            ypos: RoPE position indices for y.
        """
        # Self-attention
        ...
        x = x + self.ls_y(
            self.cross_attn(self.norm2(x), y_normed, y_normed, qpos=xpos, kpos=ypos)
        )
        # FFN
