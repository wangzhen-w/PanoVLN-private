...
# SE(3) inverse utilities

def se3_inverse(T):
    """
    Compute the inverse of SE(3) transforms.
    
    Args:
        T: (..., 4, 4) SE(3) transform matrices.
    
    Returns:
        T_inv: inverse transforms with the same shape as T.
    """
    
    if torch.is_tensor(T):
        # Upcast low-precision inputs for numerical stability.
        ...
        if torch.isnan(T).any() or torch.isinf(T).any():
            logging.error("� se3_inverse: Input T contains NaN/Inf!")
            
            identity = torch.eye(4, device=T.device, dtype=torch.float32 if needs_conversion else original_dtype)
            result = identity.reshape(*([1] * (T.ndim - 2)), 4, 4).expand_as(T)
            return result.to(original_dtype) if needs_conversion else result
        
        # For SO(3), inverse is transpose.
        R_inv = R.transpose(-2, -1)
        
        # Optional orthogonality check can be enabled for debugging.
        # RTR = torch.matmul(R.transpose(-2, -1), R)
        # I = torch.eye(3, device=R.device, dtype=R.dtype)
        # orth_error = (RTR - I).abs().max()
        # if orth_error > 0.1:
        # logging.warning(f"Large orthogonality error: {orth_error:.6f}")
        
        # Closed-form inverse translation.
        t_inv = -torch.matmul(R_inv, t)
        
        # Clamp to avoid exploding values in corrupted inputs.
        t_inv = torch.clamp(t_inv, -1e6, 1e6)
        
        # Guard against invalid outputs.
        if torch.isnan(t_inv).any() or torch.isinf(t_inv).any():
            logging.error("� se3_inverse: t_inv contains NaN/Inf!")
            logging.error(f"   t range: [{t.min():.4f}, {t.max():.4f}]")
            identity = torch.eye(4, device=T.device, dtype=torch.float32 if needs_conversion else original_dtype)
            result = identity.reshape(*([1] * (T.ndim - 2)), 4, 4).expand_as(T)
            return result.to(original_dtype) if needs_conversion else result
