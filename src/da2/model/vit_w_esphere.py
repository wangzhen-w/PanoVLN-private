...
class ViT_w_Esphere(nn.Module):
    ...
    def _prepare_decoder_inputs(self, images, features, sphere_dirs):
        ...
        sphere_embeddings = self._vit_w_esphere.embed_sphere_dirs(sphere_dirs)
        return features, sphere_embeddings

    def extract_decoder_features(self, images, features, sphere_dirs):
        features, sphere_embeddings = self._prepare_decoder_inputs(
            images,
            features,
            sphere_dirs,
        )
        return self._vit_w_esphere.process(features, sphere_embeddings)
    ...
    def forward(self, images, features, sphere_dirs) -> torch.Tensor:
        ...
        features = self._vit_w_esphere.process(features, sphere_embeddings)
        logdistance = self._vit_w_esphere.prediction_head(features)
