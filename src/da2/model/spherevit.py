...
class SphereViT(nn.Module, PyTorchModelHubMixin):
    ...
    def _encode_panorama(self, images):
        ...
        sphere_config['width'] = W_new
        sphere_config['height'] = H_new
        sphere = Sphere(config=sphere_config, device=images.device)
        ...
        sphere_dirs = sphere_dirs.to(images.device)
        ...
        return images, features, sphere_dirs, (H, W)

    def extract_decoder_features(self, images):
        images, features, sphere_dirs, _ = self._encode_panorama(images)
        return self.vit_w_esphere.extract_decoder_features(
            images,
            features,
            sphere_dirs,
        )
    ...
    def forward(self, images):
        images, features, sphere_dirs, original_shape = self._encode_panorama(images)
        distance = self.vit_w_esphere(images, features, sphere_dirs)
        distance = F.interpolate(distance, size=original_shape, mode='bilinear', align_corners=False)
